...
Kalendae.prototype = {
	...
	do {
		...
		else if (!this.direction(day) || s>0) klass.push(classes.dayActive);
		if (day.yearDay() === today.yearDay()) klass.push(classes.dayToday);

		dateString = day.format(this.settings.dayAttributeFormat);
                klass.push(dateString);
		...
		day.add('days',1);
            ...
            _this = this;
            this.blackout(month, function(date) {
                console.log('blackout callback for ' + date.format());
                $('.' + date.format(_this.settings.dayAttributeFormat)).removeClass(classes.dayActive).addClass(classes.blackedOut)}); // asynchronous blackout for whole month
	    month.add('months',1);
	} while (++i < c);
